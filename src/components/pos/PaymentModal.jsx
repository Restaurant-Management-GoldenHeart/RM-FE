  ...
  const handlePay = async () => {
    ...
    try {
      const payload = {
        ...
        taxRate,
        method,
        total: Math.round(total),  // ← truyền tổng tiền thực để BE đóng bill
      };

      await paymentApi.payOrder(payload);
      // Nếu không throw exception → thanh toán thành công
      // (BE trả về { message, data } không có field 'success')
      ...
      // 2. Xoà order khỏi store hoạt động
      ...
      // Smart recovery: khi có lỗi "recorded payments" → check lại bàn từ BE
      // Có thể lần trước đã thanh toán thành công nhưng FE cleanup bị fail
      if (beMsg.includes('recorded payments')) {
        try {
          await useTableStore.getState().fetchTables();
          const freshTable = useTableStore.getState().tables[table.id];
          // Nếu bàn không còn OCCUPIED → đã được thanh toán rồi, chỉ cần cleanup FE
          if (freshTable && freshTable.status !== 'OCCUPIED') {
            toast.success('Đơn hàng đã được thanh toán. Đang dọn dẹp...');
            useOrderStore.getState().clearOrder(order.id);
            useKitchenStore.getState().clearOrderFromKitchen(order.id);
            useTableStore.getState().setCurrentOrderTarget({ type: null, id: null, name: null });
            onClose();
        ...
        // Bàn vẫn OCCUPIED → bill thực sự bị stuck, cần quản lý xử lý
        toast.error('⚠️ Hóa đơn đã có thanh toán một phần. Vui lòng liên hệ quản lý để reset đơn hàng.', { duration: 8000 });
      } else if (beMsg.includes('not served') || beMsg.includes('all dishes')) {
        toast.error('Còn món chưa được phục vụ. Vui lòng trả đủ món trước khi thanh toán.');
      } else if (beMsg.includes('no billable items')) {
        toast.error('Đơn hàng không có món nào để tính tiền.');
      } else if (beMsg.includes('negative')) {
        toast.error('Tổng tiền không hợp lệ. Vui lòng kiểm tra giảm giá.');
      } else {
        toast.error(beMsg || 'Thanh toán thất bại. Vui lòng thử lại.', { duration: 6000 });
      }
    } finally {
      setIsPaying(false);
    }
