...
export default function TakeawayCard({ order, isActive, onClick, onAction }) {
  const getStatusStyle = (status) => {
    switch (status) {
      case 'OCCUPIED': return { color: 'text-amber-700', bg: 'bg-amber-100', border: 'border-amber-400', text: 'Đang phục vụ', dot: 'bg-amber-500' };
      case 'PENDING': return { color: 'text-orange-700', bg: 'bg-orange-100', border: 'border-orange-400', text: 'Chờ xử lý', dot: 'bg-orange-500' };
  ...
  if (isActive) {
    ...
    // ĐANG PHỤC VỤ: Nền vàng nhạt nổi bật, viền amber đặc
    cardClasses += "border-2 border-amber-400 bg-amber-50 hover:shadow-md";
  } else {
    ...
    <div onClick={onClick} className={cardClasses}>
      ...
      <div className="flex justify-center items-center gap-1.5 w-full mt-auto">
        <div className="flex items-center gap-1 bg-slate-50 px-1.5 py-1 rounded text-slate-500 font-medium text-[9px] sm:text-[10px] max-w-[50%] truncate">
          ...
          <span className="truncate">{order.customerName || order.customer_name}</span>
        </div>
