...
const getPaymentIcon = (methods) => {
  ...
  if (method.includes('CASH') || method.includes('TIEN MAT')) return Banknote;
  ...
  const fetchHistory = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        setBills(Array.isArray(payload?.content) ? payload.content : []);
        setPagination({
          page: currentPage,
          totalPages,
          totalElements,
          start: totalElements === 0 ? 0 : currentPage * pageSize + 1,
          end: totalElements === 0 ? 0 : Math.min((currentPage + 1) * pageSize, totalElements),
        });
      } catch (error) {
      ...
      <section className="premium-card overflow-hidden">
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-5">
          <div className="min-w-0">
            <p className="text-xs font-black uppercase tracking-[0.28em] text-slate-400">Lịch sử hóa đơn</p>
          </div>
          <button
            type="button"
