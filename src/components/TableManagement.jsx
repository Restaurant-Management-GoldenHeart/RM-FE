...
const TableActionModal = ({ table, onClose, onSelect }) => {
  const { openTable, cleanTable, reserveTable, mergeTables, cancelOrder, tables } = useTableStore();
  ...
  const handleCloseTable = async () => {
    setLoading(true);
    try {
      await cancelOrder(table.currentOrderId || table.id);
      toast.success(`Đã đóng bàn ${table.tableNumber}`);
      onClose();
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Không thể đóng bàn');
    } finally {
      setLoading(false);
    }
  };
