...
const STATUS_CONFIG = {
  ...
  CLEANING:  { card: 'bg-gray-50 border-gray-200 hover:border-gray-400', badge: 'bg-gray-200 text-gray-600 border-gray-300', dot: 'bg-gray-400', label: 'Cần dọn' },
};
        ...
        {view === 'MAIN' && (
          <div className="space-y-3">
            ...
            {table.status === 'OCCUPIED' && (
              <>
                <button onClick={() => onSelect(table.id)} className="w-full py-4 bg-amber-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-amber-700 transition-all">
...
export const TableList = ({ selectedTableId, onTableSelect }) => {
  ...
  const stats = useMemo(() => ({ total: tables.length, avail: tables.filter(t => t.status === 'AVAILABLE').length, occu: tables.filter(t => t.status === 'OCCUPIED').length, cleaning: tables.filter(t => t.status === 'CLEANING').length }), [tables]);
