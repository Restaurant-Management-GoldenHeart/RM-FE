...
const TableActionModal = ({ table, onClose, onSelect }) => {
  const { openTable, cleanTable, reserveTable, mergeTables, cancelOrder, tables, updateTableLocal } = useTableStore();
  const [view, setView] = useState('MAIN'); // MAIN, RESERVE, MERGE, CLOSE_CONFIRM
  ...
  // Reset view về MAIN mỗi khi đổi bàn hoặc mở lại modal
  React.useEffect(() => {
  ...
  const handleCloseTable = async () => {
    ...
    try {
      if (!table.currentOrderId) {
        // CASE: Bàn mới mở (Lazy Open), chưa có đơn hàng dưới BE
        // Chỉ cần cập nhật LOCAL về AVAILABLE, không gọi API để tránh lỗi 409
        updateTableLocal(table.id, { status: 'AVAILABLE' });
        toast.success(`Đã đóng bàn ${table.tableNumber}`);
      } else {
        // CASE: Đã có đơn hàng thật, gọi API huỷ
        ...
        {view === 'MAIN' && (
          <div className="space-y-3">
            ...
            {table.status === 'OCCUPIED' && (
              <>
                <button onClick={() => { onSelect({ table, orderId: table.currentOrderId }); onClose(); }} className="w-full py-4 bg-amber-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-amber-700 transition-all">
                  <Sparkles size={18} /> Đi tới Đơn hàng
                </button>
        ...
        {view === 'CLOSE_CONFIRM' && (
          <div className="space-y-4">
            <div className="p-4 bg-red-50 border border-red-100 rounded-2xl">
              <p className="text-sm font-black text-red-700 text-center">⚠️ Xác nhận đóng bàn?</p>
              <p className="text-[11px] text-red-500 font-bold text-center mt-1">
                Toàn bộ món chưa xử lý sẽ bị huỷ. Bàn sẽ trở về trạng thái Trống.
              </p>
            </div>
            <div className="flex gap-2 pt-2">
              <button onClick={() => setView('MAIN')} className="flex-1 py-3 text-sm font-black text-gray-400 hover:bg-gray-50 rounded-2xl">Quay lại</button>
              <button 
...
export const TableList = ({ currentOrderTarget, onTableSelect }) => {
  ...
  // Tải danh sách khu vực khi component mount (nếu chưa có)
  useMemo(() => {
  ...
  const handleTakeawayClick = async (slot) => {
    if (slot.status === 'AVAILABLE') {
      ...
      onTableSelect({ 
        table: { 
          id: slot.id, 
          tableNumber: slot.order_number || 'Mang về', 
  ...
  const handleConfirmOpeningTakeaway = async (customerName) => {
    ...
    if (res) {
      onTableSelect({ 
        table: { 
          id: openingTakeawaySlot.id, 
          tableNumber: openingTakeawaySlot.order_number || 'Mang về', 
    ...
    <div className="flex flex-col h-full bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden text-gray-900">
      ...
      <div className="p-6 border-b border-gray-50">
        <div className="flex items-center justify-between mb-6">
          ...
          {mode === 'DINE_IN' && (
            <button onClick={() => fetchTables()} className="p-2.5 hover:bg-gold-50 rounded-xl text-gray-400 hover:text-gold-600 transition-all text-gray-400">
              <TrendingUp size={18} />
            </button>
          )}
        </div>
