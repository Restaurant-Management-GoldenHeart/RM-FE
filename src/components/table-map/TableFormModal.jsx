...
import { tableApi } from '../../api/posApi';
...
const TableFormModal = ({ isOpen, onClose, initialData = null, onSuccess, areas: areasProp }) => {
  ...
  const areas = (areasProp && areasProp.filter(a => a.id !== 'ALL').length > 0)
    ? areasProp
    : storeAreas;
  ...
  const [formData, setFormData] = useState({
    tableNumber: '',
    capacity: 4,
    area_id: areas.find(a => a.id !== 'ALL')?.id || 1
  });

  useEffect(() => {
    // Only fetch from store if no areas are passed from parent
    if (isOpen && !areasProp && areas.length <= 1) {
      fetchAreas();
    }
  }, [isOpen, areasProp, areas.length, fetchAreas]);
