...
export default function TableMapPage() {
  const { 
    tables, areas, loading, isAdmin, isRootAdmin, 
    fetchTables, fetchAreas, deleteTable, deleteArea 
  } = useTableMapAdapter();
  ...
  const handleAddTable = () => {
    setEditingTable({ 
      area_id: selectedArea !== 'ALL' ? selectedArea : (areas.find(a => a.id !== 'ALL')?.id || null) 
    });
    setIsFormOpen(true);
  };
  ...
  const handleDeleteTable = (table) => {
    if (table.status === 'OCCUPIED') {
      toast.error(`Bàn ${table.tableNumber || table.table_number} đang có khách, không thể xóa!`);
      return;
    }
      ...
      <div className="flex-1 bg-gray-50 rounded-3xl xl:rounded-[2.5rem] shadow-inner border border-gray-200 overflow-hidden relative p-0">
        {loading ? (
          ...
          <FloorPlanView 
            ...
            onDelete={isRootAdmin ? handleDeleteTable : undefined}
